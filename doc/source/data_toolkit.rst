...
:mod:`data_toolkit.raw_data.pudl.download_data_from_pudl` and
:mod:`data_toolkit.raw_data.eia930a` below for the details, including the
...
See :mod:`data_toolkit.raw_data.pudl.pudl_to_gridpath_raw_data` below
...
:mod:`data_toolkit.load_raw_data` below), which reads a
...
:mod:`data_toolkit.raw_data.patch_eia_baa_codes` below; reruns are
no-ops). If you plan to use your own load-zone definitions (the
``custom`` load-zone level below), also run the ``apply_custom_zones``
step (:mod:`data_toolkit.raw_data.apply_custom_zones`) to record your
...
1. **Where** (geographic scope) — ``footprint`` filters the raw data to
   ...
   (see :mod:`data_toolkit.list_footprint_options` below).
...
2. **What** (fleet selection) — ``study_year`` anchors the fleet: units
   ...
   ``include_planned_retirements``, and ``include_btm_plants`` settings
   ...
   <data_toolkit.project.portfolios.eia860_to_project_portfolio_input_csvs>`
   below.
...
:mod:`data_toolkit.project.fleet.fleet_audit` below).
...
.. code-block:: text
    ...
    create_database,db_schema,PATH/TO/GRIDPATH/data_toolkit/raw_data_db_schema.sql,,
...
:mod:`data_toolkit.project.manual_adjustments` below) — must run LAST.
...
EIA-930A
********
.. automodule:: data_toolkit.raw_data.eia930a
...
.. automodule:: data_toolkit.raw_data.patch_eia_baa_codes
.. automodule:: data_toolkit.raw_data.apply_custom_zones
...
.. automodule:: data_toolkit.list_footprint_options
...
:mod:`data_toolkit.project.fleet` package documentation below describes
...
.. automodule:: data_toolkit.project.fleet
.. automodule:: data_toolkit.project.fleet.fleet_audit
...
.. automodule:: data_toolkit.project.capacity_specified.eia860m_to_project_specified_capacity_input_csvs
...
.. automodule:: data_toolkit.project.manual_adjustments
