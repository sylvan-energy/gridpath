...
# Note that we are checking sod_stor inputs have been added by the
...
class TestSodStor(unittest.TestCase):
    """ """
    ...
    def test_data_loaded_correctly(self):
        ...
        # Set: STOR_PROJECT_POLICY_ZONES
        expected_stor_ppz = sorted(
            [
                ("Battery", "SLICE_OF_DAY", "SODZone1"),
            ]
        )
        actual_stor_ppz = sorted(
            [
                (prj, policy, zone)
                for (prj, policy, zone) in instance.STOR_PROJECT_POLICY_ZONES
            ]
        )
        self.assertListEqual(expected_stor_ppz, actual_stor_ppz)

        # Param: policy_stor_round_trip_efficiency
        expected_rte = OrderedDict(
            sorted(
                {
                    ("Battery", "SLICE_OF_DAY", "SODZone1"): 0.7,
                }.items()
            )
        )
        actual_rte = OrderedDict(
            sorted(
                {
                    (prj, policy, zone): instance.policy_stor_round_trip_efficiency[
                        prj, policy, zone
                    ]
                    for (prj, policy, zone) in instance.STOR_PROJECT_POLICY_ZONES
                }.items()
        ...
        self.assertDictEqual(expected_rte, actual_rte)
