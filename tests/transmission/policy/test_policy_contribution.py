...
NAME_OF_MODULE_BEING_TESTED = "transmission.policy.policy_contribution"
...
class TestTxPolicyContribution(unittest.TestCase):
    """ """
    ...
    def test_data_loaded_correctly(self):
        ...
        # Set: TX_POLICY_ZONES
        expected_tx_policy_zones = sorted(
            [
                ("Tx_New", "RPS", "RPSZone1"),
                ("Tx_binary_1", "RPS", "RPSZone1"),
            ]
        )
        actual_tx_policy_zones = sorted(
            [(tx, policy, zone) for (tx, policy, zone) in instance.TX_POLICY_ZONES]
        )
        self.assertListEqual(expected_tx_policy_zones, actual_tx_policy_zones)

        # Param: tx_compliance_type
        expected_compliance_type = OrderedDict(
            sorted(
                {
                    ("Tx_New", "RPS", "RPSZone1"): "tx_losses",
                    ("Tx_binary_1", "RPS", "RPSZone1"): "tx_losses",
                }.items()
            )
        )
        actual_compliance_type = OrderedDict(
            sorted(
                {
                    (tx, policy, zone): instance.tx_compliance_type[tx, policy, zone]
                    for (tx, policy, zone) in instance.TX_POLICY_ZONES
                }.items()
            )
        )
