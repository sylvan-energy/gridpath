...
def _get_hybrid_pairing_subqueries(generators_table, fleet_relation_sql):
    ...
    return fleet_batteries_sql, fleet_var_units_sql, get_support_links_sql()


def get_support_links_sql():
    """
    The batteries' direct-support links from the EIA860 energy-storage
    supplement as one relation — all three link slots unioned into
    (batt_plant, batt_gen, supported_plant, supported_gen) rows, cross-plant
    links included, with no fleet filtering (callers restrict both ends to
    their fleet). Used by the pairing expressions and by the hybrid
    power-output-group step's clustering.
    """
    return " UNION ".join(f"""
...
def get_hybrid_var_component_condition(generators_table, fleet_relation_sql):
    """
    SQL boolean: the *generators_table* row is the VARIABLE (solar/wind)
    component of a hybrid pair — same pairing rules as
    get_hybrid_pairing_expr, batteries never match. Under the
    power_output_group hybrid treatment, the opchar step flips these
    units' key operational type from gen_var_must_take to gen_var: a
    must-take unit cannot be curtailed, so a plant whose variable
    component alone can exceed the shared interconnection limit would make
    the group's max constraint infeasible.
    """
    (
        _battery_link_sql,
        _battery_colocation_sql,
        var_link_sql,
        var_colocation_sql,
    ) = _get_hybrid_pairing_branches(
        generators_table=generators_table,
    ...
    return f"(({var_link_sql}) OR ({var_colocation_sql}))"


# How hybrid components are modeled. 'independent' (the default) keeps
# them as unrelated projects — today's behavior. 'power_output_group' keeps
# them as separate projects but couples each hybrid's components with a
# GridPath power output group capping their COMBINED output at the shared
# interconnection (POI) limit (generated by the
# eia860_to_project_power_output_group_input_csvs step; the opchar step
# flips paired variable components to the curtailable gen_var type — see
# get_hybrid_var_component_condition). A future 'gen_var_stor_hyb'
# treatment (one merged project per hybrid, field-only charging) is
# designed but not implemented. One treatment applies fleet-wide: pairing
# is per-plant data, but mixed treatments would break zone-level
# aggregation (an aggregate cannot mix merged and grouped components).
HYBRID_TREATMENT_CHOICES = ("independent", "power_output_group")


def add_hybrid_treatment_argument(parser):
    """
    Add the shared ``--hybrid_treatment`` argument to a project-level
    step's argument parser — single-sourced here (next to the pairing
    machinery it builds on) so the setting can't drift across the steps.
    """
    parser.add_argument(
        "-hybtr",
        "--hybrid_treatment",
        default="independent",
        choices=list(HYBRID_TREATMENT_CHOICES),
        help="How hybrid components (see the 'hybrid' aggregation "
        "dimension for the pairing rules) are modeled: 'independent' (the "
        "default) keeps them as unrelated projects; 'power_output_group' "
        "couples each hybrid's components with a power output group "
        "capping their combined output at the shared interconnection "
        "limit — the eia860_to_project_power_output_group_input_csvs step "
        "generates the group CSVs, and the opchar step flips paired "
        "variable components from gen_var_must_take to the curtailable "
        "gen_var. In an aggregated mode, a non-independent treatment "
        "requires the 'hybrid' aggregation dimension (hybrid and "
        "standalone components must not share an aggregate). Must be set "
