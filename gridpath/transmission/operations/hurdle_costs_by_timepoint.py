...
def add_model_components(
    ...
    +=========================================================================+
    | | :code:`hurdle_rate_by_tmp_pos_dir_per_mwh`                                   |
    ...
    +-------------------------------------------------------------------------+
    | | :code:`hurdle_rate_by_tmp_neg_dir_per_mwh`                                   |
    | | *Defined over*: :code:`TX_OPR_TMPS`                                   |
    | | *Within*: :code:`NonNegativeReals`                                    |
    | | *Defaults to*: :code:`0`                                              |
    |                                                                         |
    ...
    | Variables                                                               |
    +=========================================================================+
    | | :code:`Hurdle_Cost_By_Tmp_Pos_Dir`                                           |
    | | *Defined over*: :code:`TX_OPR_TMPS`                                          |
    | | *Within*: :code:`NonNegativeReals`                                    |
    |                                                                         |
    ...
    +-------------------------------------------------------------------------+
    | | :code:`Hurdle_Cost_By_Tmp_Neg_Dir`                                    |
    | | *Defined over*: :code:`TX_OPR_TMPS`                                          |
    | | *Within*: :code:`NonNegativeReals`                                    |
    |                                                                         |
    ...
    | Constraints                                                             |
    +=========================================================================+
    | | :code:`Hurdle_Cost_By_Tmp_Pos_Dir_Constraint`                                |
    ...
    +-------------------------------------------------------------------------+
    | | :code:`Hurdle_Cost_By_Tmp_Neg_Dir_Constraint`                         |
    | | *Enforced over*: :code:`TX_OPR_TMPS`                                         |
    |                                                                         |
    ...
    # Optional Input Params
    ###########################################################################
    m.hurdle_rate_by_tmp_pos_dir_per_mwh = Param(
        # m.TX_LINES,
        m.TX_OPR_TMPS,
    ...
    m.hurdle_rate_by_tmp_neg_dir_per_mwh = Param(
        # m.TX_LINES,
        m.TX_OPR_TMPS,
        within=NonNegativeReals,
        default=0,
    ...
    m.Hurdle_Cost_By_Tmp_Pos_Dir = Var(m.TX_OPR_TMPS, within=NonNegativeReals)
    m.Hurdle_Cost_By_Tmp_Neg_Dir = Var(m.TX_OPR_TMPS, within=NonNegativeReals)

    # Constraints
    ###########################################################################

    m.Hurdle_Cost_By_Tmp_Pos_Dir_Constraint = Constraint(
        m.TX_OPR_TMPS, rule=hurdle_cost_by_tmp_pos_dir_rule
    )

    m.Hurdle_Cost_By_Tmp_Neg_Dir_Constraint = Constraint(
        m.TX_OPR_TMPS, rule=hurdle_cost_by_tmp_neg_dir_rule
    )
...
def hurdle_cost_by_tmp_pos_dir_rule(mod, tx, tmp):
    """
    **Constraint Name**: Hurdle_Cost_By_Tmp_Pos_Dir_Constraint
    **Enforced Over**: TX_OPR_TMPS

    Hurdle_Cost_By_Tmp_Pos_Dir must be non-negative, so will be 0
    when Transmit_Power is negative (flow in the negative direction).
    """
    if mod.hurdle_rate_by_tmp_pos_dir_per_mwh[tx, tmp] == 0:
        return Constraint.Skip
    else:
        return (
            mod.Hurdle_Cost_By_Tmp_Pos_Dir[tx, tmp]
            >= mod.Transmit_Power_MW[tx, tmp]
            * mod.hurdle_rate_by_tmp_pos_dir_per_mwh[tx, tmp]
        )


def hurdle_cost_by_tmp_neg_dir_rule(mod, tx, tmp):
    """
    **Constraint Name**: Hurdle_Cost_By_Tmp_Neg_Dir_Constraint
    **Enforced Over**: TX_OPR_TMPS

    Hurdle_Cost_By_Tmp_Neg_Dir must be non-negative, so will be 0
    when Transmit_Power is positive (flow in the positive direction).
    """
    if mod.hurdle_rate_by_tmp_neg_dir_per_mwh[tx, tmp] == 0:
        return Constraint.Skip
    else:
        return (
            mod.Hurdle_Cost_By_Tmp_Neg_Dir[tx, tmp]
            >= -mod.Transmit_Power_MW[tx, tmp]
            * mod.hurdle_rate_by_tmp_neg_dir_per_mwh[tx, tmp]
        )
...
def load_model_data(
    ...
    data_portal.load(
        filename=os.path.join(
            ...
            stage,
            "inputs",
            "transmission_hurdle_rates_by_timepoint.tab",
        ),
        select=(
            "transmission_line",
            "timepoint",
...
def export_results(
    ...
    results_columns = [
        "hurdle_cost_by_timepoint_positive_direction",
...
def get_inputs_from_database(
    ...
    hurdle_rates = c.execute(
        ...
        LEFT OUTER JOIN
            ...
            hurdle_rate_by_timepoint_positive_direction_per_mwh,
            hurdle_rate_by_timepoint_negative_direction_per_mwh
            FROM inputs_transmission_hurdle_rates_by_timepoint
            WHERE transmission_hurdle_rate_by_timepoint_scenario_id = {}) AS relevant_hrs
        USING (transmission_line, timepoint)
        WHERE transmission_portfolio_scenario_id = {};
        """.format(
            subscenarios.TEMPORAL_SCENARIO_ID,
            subscenarios.TRANSMISSION_HURDLE_RATE_BY_TIMEPOINT_SCENARIO_ID,
...
def write_model_inputs(
    ...
    transmission_hurdle_rates_by_timepoint.tab file.
    ...
    hurdle_rates_by_timepoint = get_inputs_from_database(
        scenario_id,
        subscenarios,
        db_weather_iteration,
        db_hydro_iteration,
    ...
    with open(
        ...
        for row in hurdle_rates_by_timepoint:
...
def process_results(db, c, scenario_id, subscenarios, quiet):
    ...
    # Delete old results
    del_sql = """
        DELETE FROM results_transmission_hurdle_costs_by_timepoint_agg
        WHERE scenario_id = ?
        """
...
def validate_inputs(
    ...
    df = cursor_to_df(hurdle_rates_by_timepoint)

    # Get expected dtypes
    expected_dtypes = get_expected_dtypes(
        conn=conn, tables=["inputs_transmission_hurdle_rates_by_timepoint"]
    )
    ...
    write_validation_to_database(
        ...
        db_table="inputs_transmission_hurdle_rates_by_timepoint",
        severity="High",
        errors=dtype_errors,
    )
    ...
    msg = (
        "Expected hurdle rates specified for each modeling timepoint when"
        "transmission hurdle rates by timepoint feature is on."
    )
    cols = [
        "hurdle_rate_by_timepoint_positive_direction_per_mwh",
        "hurdle_rate_by_timepoint_negative_direction_per_mwh",
