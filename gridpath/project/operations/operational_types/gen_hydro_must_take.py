...
def add_model_components(
    ...
    +-------------------------------------------------------------------------+
    | | :code:`GEN_HYDRO_MUST_TAKE_BUDGET_ALLOC_BT_HRZS`                      |
    |                                                                         |
    | Three-dimensional set with generators of the :code:`gen_hydro_must_take`|
    ...
    +-------------------------------------------------------------------------+
    | | :code:`gen_hydro_must_take_budget_alloc_min_fraction`                 |
    ...
    +-------------------------------------------------------------------------+
    | | :code:`gen_hydro_must_take_budget_alloc_max_fraction`                 |
    | | *Defined over*: :code:`GEN_HYDRO_MUST_TAKE_BUDGET_ALLOC_BT_HRZS`      |
    | | *Within*: :code:`PercentFraction`                                     |
    |                                                                         |
    ...
    | Derived Params                                                          |
    +=========================================================================+
    | | :code:`gen_hydro_must_take_budget_alloc_parent_hrz`                   |
    | | *Defined over*: :code:`GEN_HYDRO_MUST_TAKE_BUDGET_ALLOC_BT_HRZS`      |
    ...
    +-------------------------------------------------------------------------+
    | | :code:`GenHydroMustTake_Parent_Hrz_Energy_Budget_MWh`                 |
    | | *Defined over*:                                                       |
    | | :code:`GEN_HYDRO_MUST_TAKE_BUDGET_ALLOC_PARENT_BT_HRZS`               |
    |                                                                         |
    ...
    +-------------------------------------------------------------------------+
    | | :code:`GenHydroMustTake_Budget_Alloc_Energy_MWh`                      |
    | | *Defined over*: :code:`GEN_HYDRO_MUST_TAKE_BUDGET_ALLOC_BT_HRZS`      |
    |                                                                         |
    ...
    | :code:`gen_hydro_must_take_average_power_fraction`.                     |
    +-------------------------------------------------------------------------+
    | | :code:`GenHydroMustTake_Budget_Alloc_Min_Constraint`                  |
    | | *Defined over*: :code:`GEN_HYDRO_MUST_TAKE_BUDGET_ALLOC_BT_HRZS`      |
    |                                                                         |
    | The project's energy in the sub-horizon must be at least the            |
    | specified :code:`gen_hydro_must_take_budget_alloc_min_fraction`         |
    | of the parent horizon's energy budget.                                  |
    +-------------------------------------------------------------------------+
    | | :code:`GenHydroMustTake_Budget_Alloc_Max_Constraint`                  |
    | | *Defined over*: :code:`GEN_HYDRO_MUST_TAKE_BUDGET_ALLOC_BT_HRZS`      |
    |                                                                         |
    | The project's energy in the sub-horizon must be at most the             |
    | specified :code:`gen_hydro_must_take_budget_alloc_max_fraction`         |
    | of the parent horizon's energy budget.                                  |
    +-------------------------------------------------------------------------+
    | Ramps                                                                   |
    +-------------------------------------------------------------------------+
    ...
    m.gen_hydro_must_take_max_power_fraction = Param(
        m.GEN_HYDRO_MUST_TAKE_OPR_BT_HRZS, within=Reals
    )
    ...
    m.GenHydroMustTake_Energy_Budget_Constraint = Constraint(
        m.GEN_HYDRO_MUST_TAKE_OPR_BT_HRZS, rule=energy_budget_rule
    )
    ...
    add_hydro_budget_allocation_components(
        ...
        component_prefix="GenHydroMustTake",
        power_var="GenHydroMustTake_Gross_Power_MW",
    )
...
def energy_budget_rule(mod, g, bt, h):
    ...
    **Enforced Over**: GEN_HYDRO_MUST_TAKE_OPR_BT_HRZS
    ...
    return sum(
        ...
        mod.gen_hydro_must_take_average_power_fraction[g, bt, h]
        * mod.Capacity_MW[g, mod.period[tmp]]
        * mod.Availability_Derate[g, tmp]
        * mod.hrs_in_tmp[tmp]
        for tmp in mod.TMPS_BY_BLN_TYPE_HRZ[bt, h]
