...
0. **BA assignment** (:mod:`data_toolkit.project.fleet.ba_assignment`) —
...
1. **Geographic scope** (:mod:`data_toolkit.geographic_scope`, which lives
...
2. **Fleet filters** (:mod:`data_toolkit.project.fleet.fleet_filters`) —
...
3. **Aggregation** (:mod:`data_toolkit.project.fleet.aggregation`) — how
...
:mod:`data_toolkit.project.fleet.step_common` holds the scaffolding every
step's ``main`` runs (resolve settings, connect, run the scope checks and
data-gap warnings), and :mod:`data_toolkit.project.fleet.fleet_audit` is
...
Note that :mod:`data_toolkit.project.project_data_filters_common`, one
